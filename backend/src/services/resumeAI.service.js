...
function basicResumeProfile(resumeText) {
  ...
  return {
    headline:
      "Resume uploaded — add a Groq API key for full AI profile extraction.",
    location: "",
    linkedIn,
    ...
    messages: [
      {
        role: "system",
        content:
          "Extract resume data. Return only JSON with headline, location, linkedIn, skills (string array), education ({degree,institution,year} array), experience ({title,company,duration} array). Never invent facts.",
      },
      { role: "user", content: resumeText.slice(0, 24000) },
    ],
  });
  const data = jsonFromResponse(
...
export function scoreJob(resumeSkills = [], job) {
  ...
  const candidateSkills = [
    ...new Set(resumeSkills.map(normalize).filter(Boolean)),
  ];
  const jobSkills = [
    ...new Set((job.skills || []).map(normalize).filter(Boolean)),
  ];
  ...
  return {
    ...
    summary: matched.length
      ? `Strongest overlap: ${matched.slice(0, 4).join(", ")}.`
      : "Add role-specific skills to improve this match.",
  };
}
